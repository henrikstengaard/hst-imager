...
public static class MbrTestHelper
{
    ...
    {
        var fileSystem = await MountFileSystem(testCommandHelper, path, 0, true);
    ...
    public static async Task<IFileSystem> MountFileSystem(TestCommandHelper testCommandHelper, string mediaPath,
    ...
    {
        var fileSystem = await MountFileSystem(testCommandHelper, mediaPath, partitionNumber, writable);
