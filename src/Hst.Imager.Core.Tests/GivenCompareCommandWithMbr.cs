...
public class GivenCompareCommandWithMbr : CommandTestBase
{
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task When_CompareSrcToDestMbrDisk_Then_DataIsIdentical(bool skipZeroFilled)
    {
        ...
        await TestHelper.AddMbrDiskPartition(testCommandHelper, srcPath, 20.MB().ToSectorSize());
        await TestHelper.AddMbrDiskPartition(testCommandHelper, srcPath, 40.MB().ToSectorSize());

        // arrange - create dest mbr disk cloned from src mbr disk
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task When_CompareSrcToZipCompressedDestMbrDisk_Then_DataIsIdentical(bool skipZeroFilled)
    {
        // arrange - create src and dest paths
        var srcPath = $"src-{Guid.NewGuid()}.vhd";
        ...
        try
        {
            ...
            // arrange - create zip compressed dest mbr disk cloned from src mbr disk
            var srcData = await TestHelper.ReadData(testCommandHelper, srcPath);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task When_CompareZipCompressedSrcToDestMbrDisk_Then_DataIsIdentical(bool skipZeroFilled)
    {
        // arrange - create src and dest paths
        ...
        try
        {
            ...
            // arrange - create zip compressed src mbr disk cloned from dest mbr disk
    ...
    [Fact]
    public async Task When_CompareSrcToDestMbrPartition1_Then_DataIsIdentical()
    {
        // arrange - create src and dest paths
        var srcPath = $"src-{Guid.NewGuid()}.vhd";
        var destPath = $"dest-{Guid.NewGuid()}.vhd";
        var comparePath = Path.Combine(destPath, "mbr", "1");
        ...
        // arrange - write data to dest mbr partition 1
        var mbrPartition1Part = await TestHelper.GetMbrPartitionPart(testCommandHelper, destPath, 1);
        await TestHelper.WriteData(testCommandHelper, destPath, mbrPartition1Part.StartOffset, srcData);
        
        // arrange - create compare command to compare mbr partition 1
        var compareCommand = new CompareCommand(new NullLogger<CompareCommand>(), testCommandHelper,
    ...
    [Fact]
    public async Task When_CompareSrcWithUnusedSectorsToDestMbrPartition1_Then_DataIsIdentical()
    {
        // arrange - create src and dest paths
        var srcPath = $"src-{Guid.NewGuid()}.vhd";
        var destPath = $"dest-{Guid.NewGuid()}.vhd";
    ...
    [Fact]
    public async Task When_CompareSrcToDestMbrPartition2_Then_DataIsIdentical()
    {
        // arrange - create src and dest paths
        var srcPath = $"src-{Guid.NewGuid()}.vhd";
        var destPath = $"dest-{Guid.NewGuid()}.vhd";
        var comparePath = Path.Combine(destPath, "mbr", "2");
        ...
        // arrange - write data to dest mbr partition 2
        var mbrPartition1Part = await TestHelper.GetMbrPartitionPart(testCommandHelper, destPath, 2);
        await TestHelper.WriteData(testCommandHelper, destPath, mbrPartition1Part.StartOffset, srcData);
        
        // arrange - create compare command to compare mbr partition 2
        var compareCommand = new CompareCommand(new NullLogger<CompareCommand>(), testCommandHelper,
            [], srcPath, 0, comparePath, 0, new Size(0, Unit.Bytes), 0, false,
            false);
    ...
    [Fact]
    public async Task When_CompareSrcLargerThanDestToDestMbrPartition_Then_ErrorIsReturned()
    {
        // arrange - create src and dest paths
        var srcPath = $"src-{Guid.NewGuid()}.vhd";
        var destPath = $"dest-{Guid.NewGuid()}.vhd";
