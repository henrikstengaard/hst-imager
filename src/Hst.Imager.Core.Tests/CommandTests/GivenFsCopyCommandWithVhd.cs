...
public class GivenFsCopyCommandWithVhd : FsCommandTestBase
{
    ...
    public async Task WhenCopyingFromAndToSameVhdThenDirectoriesAreCopied()
    {
        ...
        var srcPath = Path.Combine(imagePath, "rdb", "dh0", "dir1");
    ...
    [Fact]
    public async Task When_CopyingFromAndToSameVhdWithCyclicPaths_Then_ErrorIsReturned()
    {
        var imagePath = $"{Guid.NewGuid()}.vhd";
        ...
        try
        {
            ...
            // assert - copy command failed with cyclic path error
