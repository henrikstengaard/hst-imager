...
public class GivenFsExtractCommandWithLzxContainingSpecialCharacters
{
    [Fact]
    public async Task When_ExtractingLzxWithUaeFsDbMetadata_Then_LzxIsExtractedWithUaeMetadata()
    {
        // arrange - paths
        ...
        try
        {
            ...
            var fsExtractCommand = new FsExtractCommand(new NullLogger<FsExtractCommand>(), testCommandHelper,
                [], srcPath, destPath, true, true, true, true, true,
                UaeMetadata.UaeFsDb);
    ...
    [Fact]
    public async Task When_ExtractingSingleFileFromLzxWithUaeFsDbMetadata_Then_FileIsExtractedWithUaeMetadata()
    {
        // arrange - paths
        var lzxPath = Path.Combine("TestData", "Lzx", "special_chars.lzx");
        var srcPath = $"{Guid.NewGuid()}.lzx";
